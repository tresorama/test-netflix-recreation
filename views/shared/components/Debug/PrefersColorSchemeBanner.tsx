...
const usePrefersColorScheme = () => {
  ...
  const [preferredColorSchema, setPreferredColorSchema] = useState<'dark' | 'light' | 'no-preference'>('no-preference');
  ...
  React.useEffect(() => {
    ...
    if (typeof isLight.addListener === 'function') {
      ...
      const listener = () => setPreferredColorSchema(
        isDark.matches ? 'dark' :
          isLight.matches ? 'light' :
            'no-preference'
      );
...
const Inner = ({ ...rest }: BoxProps) => {
  ...
  return (
    <Box w="100%" bg="white" color="black" {...rest}>
      <Text textAlign="center" fontSize="xs" fontWeight="bold">Prefers Color Scheme: {preferredColorScheme}</Text>
    </Box>
  )
}